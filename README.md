...
Sixteen themes, light and dark. The WCRG-TV look is still the default and nothing changes unless you go looking, but if it was never your taste there is now Nord, Dracula, Solarized, Gruvbox, Catppuccin and the rest waiting in Settings, plus **Match system** to follow Windows. See the [changelog](CHANGELOG.md#081) for the full update.
...
- **Sixteen themes, light and dark.** The default is the app's own WCRG-TV look. If it isn't for you, there's Nord, Dracula, Tokyo Night, One Dark, Catppuccin, Gruvbox, Monokai, Rose Pine, Everforest, Solarized and GitHub Light, or **Match system** to follow Windows. Every palette is contrast-checked so small text and readouts stay readable.
...
<img src="docs/screenshots/settings.png" alt="Settings: theme, proxy cache, encoder, updates" width="820" />

<sub>**Settings** handles the theme, the preview cache, the CPU/GPU encoder, and updates.</sub>
...
Themes are CSS custom properties. `renderer/brand/themes.css` and `themes.json` are
generated by `node scripts/build-themes.js` from the palette anchors in that script —
edit the anchors, not the CSS. The generator solves each theme's ink ladder against
its own panel to hit the contrast floors, and `tests/themes.test.js` re-checks the
committed CSS and fails if it has drifted from the generator.
